...
class Updater {
  ...
  get binPath() {
    return process.env.CLI_BINPATH || this.config.initPath;
  }
  get updateDir() {
  ...
  async fetchManifest(channel) {
    try {
      let { body } = await _httpCall2.default.get(this.s3url(channel, `${this.config.platform}-${this.config.arch}`));
      return body;
    } catch (err) {
  ...
  async fetchVersion(download) {
    ...
    if (!v) {
      ...
      let { body } = await _httpCall2.default.get(this.s3url(this.config.channel, 'version'));
      v = body;
  ...
  async update(manifest) {
    ...
    let { response: stream } = await _httpCall2.default.stream(url);
